...
public class Main implements IXposedHookLoadPackage {
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam lpparam) throws Throwable {
        ...
        try {
            ...
            XposedBridge.log("Yuri Init Error: " + e.getMessage());
        }
    }
    
    @SuppressLint({"PrivateApi", "DiscouragedPrivateApi"})
    private void showFloatingWindowOnStart() {
        try {
            ...
            java.lang.reflect.Method getSystemContextMethod = activityThreadClass.getDeclaredMethod("getSystemContext");
            Context context = (Context) getSystemContextMethod.invoke(activityThread);
            
            // 获取屏幕尺寸
            getScreenSize(context);
            
            createInAppFloatingWindow(context);
            
        } catch (Exception e) {
            XposedBridge.log("Yuri System Context Error: " + e.getMessage());
        }
    }
    ...
    private void createInAppFloatingWindow(Context context) {
        if (floatingView != null) return;
        
        try {
            // 计算悬浮窗尺寸 - 横屏宽度的1/3
            int floatingWidth = screenWidth / 3;
            int floatingHeight = 280; // 固定高度，适合横屏显示
